...
class MasterService {
  ...
  async getMasters(req, res) {
    ...
    const masters = await db.query('SELECT id, name, rating, city_id FROM masters');
    res.json(masters.rows);
  };
  async getMasterById(req, res) {
    logger.trace("getMasterById()");
    const id = req.params.id;
    const master = await db.query('SELECT name, rating, city_id  FROM masters where id = $1', [id]);
  ...
  async deleteMaster(req, res) {
    ...
    const masters = await db.query('SELECT id, name, rating, id, city_id FROM masters where city_id = $1', [id]);
    res.json(masters.rows);
  };
}
